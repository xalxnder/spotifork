...
for i in range(0, 400, 100):
	playlist_tracks_request = requests.get(
		'https://api.spotify.com/v1/playlists/' + PLAYLIST_ID + '/tracks?offset=' + str(i), headers=header).json()
...
Please see duplicates.csv for an example.
"""
...
for track_uri in track_df['uri']:
	if track_uri not in current_playlist_ids:
		add_request = requests.post('https://api.spotify.com/v1/playlists/' + PLAYLIST_ID + '/tracks',
		                            params='uris=' + track_uri, headers=header)
